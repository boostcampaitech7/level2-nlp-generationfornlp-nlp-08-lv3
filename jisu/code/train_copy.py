...
model = AutoModelForCausalLM.from_pretrained(
    config["model_name"], 
    torch_dtype=torch.float16 if quantization_config.get("enable_mixed_precision", False) else torch.float32,
    trust_remote_code=True,
    device_map="auto",  # 양자화 지원 장치에 자동 매핑
    load_in_8bit=True,  # bitsandbytes INT8 로딩
    quantization_config=bnb.QuantizationConfig(
        quantization_method=quantization_config.get("quantization_method", "AWQ"),
        weight_bits=quantization_config.get("weight_bits", 8),
        activation_bits=quantization_config.get("activation_bits", 8),
        calibration_method=quantization_config.get("calibration_method", "minmax")
    )
)
