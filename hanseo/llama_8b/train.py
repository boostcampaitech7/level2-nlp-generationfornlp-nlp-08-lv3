...
set_seed(config["random_seed"])  # random seed 설정
...
model = AutoModelForCausalLM.from_pretrained(
    ...
    low_cpu_mem_usage=True  # low_cpu_mem_usage 설정
)
...
# 데이터 분리 및 필터링
train_dataset, eval_dataset = filter_and_split_dataset(tokenized_train_data, max_length=config["sft_config"]["max_seq_length"], test_size=0.1, seed=config["random_seed"])
...
def preprocess_logits_for_metrics(logits, labels):
    ...
    logit_idx = [tokenizer.vocab.get(str(i), -1) for i in range(1, 6)]
    logit_idx = [idx for idx in logit_idx if idx != -1]
...
def compute_metrics(evaluation_result):
    ...
    labels = list(map(lambda x: int_output_map.get(x, -1), labels))
