...
def process_paragraph_and_question(paragraph, question):
    prompt = f"""당신은 한국사 전문가입니다. 다음 지문과 질문을 읽고, 질문에 답하기 위해 필요한 중요한 정보와 관련된 추가적인 역사적 맥락이나 배경 지식을 제공해주세요. 특히 지문에 언급된 인물, 사건, 시대 등에 대해 더 자세한 설명이 필요할 경우 그 정보를 포함해주세요.
    ...
    <힌트>
    (지문의 내용과 질문을 바탕으로 중요한 역사적 정보와 추가적인 맥락을 제공해주세요. 답변은 간결하고 명확하게 300자 이내로 작성해주세요.)
    """
...
# CSV 파일 처리 및 결과 저장
input_file = './raw_data/kmmlu.csv'
output_file = './hint/kmmlu_add_hint.csv'
