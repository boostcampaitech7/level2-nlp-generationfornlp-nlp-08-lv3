...
def process_paragraph_and_question(paragraph, question):
    prompt = f"""당신은 지리 전문가입니다. 다음 지문과 질문을 읽고, 질문에 답하기 위해 필요한 중요한 정보와 관련된 추가적인 지리적 맥락이나 배경 지식을 제공해주세요. 특히 지문에 언급된 지형, 기후, 인구, 문화, 경제 활동, 환경 문제 등에 대해 더 자세한 설명이 필요할 경우 그 정보를 포함해주세요.
    ...
    <힌트>
    (지문의 내용과 질문을 바탕으로 중요한 지리적 정보와 추가적인 맥락을 제공해주세요. 답변은 간결하고 명확하게 300자 이내로 작성해주세요. 필요한 경우 관련 지도, 통계 데이터, 지리적 현상, 또는 지역 간 비교 정보를 포함할 수 있습니다.)
    """
...
# CSV 파일 처리 및 결과 저장
input_file = './raw_data/mmmlu_geo.csv'
output_file = './hint/mmmlu_geo_add_hint.csv'
