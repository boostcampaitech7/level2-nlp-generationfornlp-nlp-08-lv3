...
def process_paragraph_and_question(paragraph, question):
    prompt = f"""당신은 정치 전문가입니다. 다음 지문과 질문을 읽고, 질문에 답하기 위해 필요한 중요한 정보와 관련된 추가적인 정치적 맥락이나 배경 지식을 제공해주세요. 특히 지문에 언급된 정치 체제, 정책, 이념, 정치적 사건, 국제 관계 등에 대해 더 자세한 설명이 필요할 경우 그 정보를 포함해주세요.
    ...
    <힌트>
    (지문의 내용과 질문을 바탕으로 중요한 정치적 정보와 추가적인 맥락을 제공해주세요. 답변은 간결하고 명확하게 300자 이내로 작성해주세요. 필요한 경우 관련 법률, 정치 이론, 역사적 사건, 또는 정치적 인물에 대한 정보도 포함할 수 있습니다.)
    """
...
# CSV 파일 처리 및 결과 저장
input_file = './raw_data/mmmlu_pol.csv'
output_file = './hint/mmmlu_pol_add_hint.csv'
