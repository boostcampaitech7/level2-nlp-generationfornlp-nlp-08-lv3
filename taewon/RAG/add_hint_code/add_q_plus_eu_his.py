...
def process_paragraph_and_question(paragraph, question):
    prompt = f"""당신은 유럽 역사학자입니다. 다음 지문과 질문을 읽고, 질문에 답하기 위해 필요한 중요한 정보와 관련된 추가적인 유럽 역사적 맥락이나 배경 지식을 제공해주세요. 특히 지문에 언급된 시대, 국가, 왕조, 전쟁, 혁명, 문화 운동, 사상가, 예술 양식 등에 대해 더 자세한 설명이 필요할 경우 그 정보를 포함해주세요.
    ...
    <힌트>
    (지문의 내용과 질문을 바탕으로 중요한 유럽 역사적 정보와 추가적인 맥락을 제공해주세요. 답변은 간결하고 명확하게 300자 이내로 작성해주세요. 필요한 경우 관련 역사적 문서, 연대기, 다른 유럽 국가들과의 관계, 당시의 사회경제적 상황, 또는 역사적 해석의 변화에 대한 정보를 포함할 수 있습니다. 유럽의 다양한 지역과 문화적 차이도 고려해주세요.)
    """
...
# CSV 파일 처리 및 결과 저장
input_file = './raw_data/mmmlu_eu_his.csv'
output_file = './hint/mmmlu_eu_his_add_hint.csv'
