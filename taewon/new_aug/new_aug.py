...
# CSV 파일 읽기
df = pd.read_csv('classified_train_processed_filtered1.csv')
...
# 결과를 CSV 파일로 저장
result_df.to_csv('new_augmented_questions.csv', index=False)

# 실패한 행들을 CSV 파일로 저장
failed_df = pd.DataFrame(failed_rows)
failed_df.to_csv('failed_rows1.csv', index=False)

print(f"데이터 증강이 완료되었습니다. 결과가 'new_augmented_questions.csv' 파일에 저장되었습니다.")
print(f"총 {len(failed_rows)}개의 행에 대해 질문 생성에 실패했습니다. 실패한 행들은 'failed_rows1.csv' 파일에 저장되었습니다.")
